#[test_only]
module aave_pool::variable_token_factory_tests {
    ...
    fun test_variable_token_initialization(
        ...
        let emitted_events = emitted_events<variable_token_factory::Initialized>();
        // make sure event of type was emitted
        assert!(vector::length(&emitted_events) == 1, TEST_SUCCESS);
        let variable_token_address = variable_token_factory::token_address(signer::address_of(variable_tokens_admin), symbol);
        let variable_token_metadata = variable_token_factory::asset_metadata(signer::address_of(variable_tokens_admin), symbol);
        assert!(object::address_to_object<Metadata>(variable_token_address)
            == variable_token_metadata,
            TEST_SUCCESS);
        assert!(variable_token_factory::get_revision() == 1, TEST_SUCCESS);
        assert!(variable_token_factory::supply(variable_token_address) == 0, TEST_SUCCESS);
        assert!(variable_token_factory::decimals(variable_token_address) == decimals,
            TEST_SUCCESS);
        assert!(variable_token_factory::maximum(variable_token_address)
            == option::none(),
            TEST_SUCCESS);
        assert!(variable_token_factory::symbol(variable_token_address) == symbol,
            TEST_SUCCESS);
        assert!(variable_token_factory::name(variable_token_address) == name, TEST_SUCCESS);
    }
    ...
    fun test_variable_token_mint_burn(
        ...
        // make sure event of type was emitted
        assert!(vector::length(&emitted_events) == 1, TEST_SUCCESS);
        let var_token_address = variable_token_factory::token_address(signer::address_of(variable_tokens_admin), symbol);
        let _var_token_metadata = variable_token_factory::get_metadata_by_symbol(signer::address_of(variable_tokens_admin), symbol);
        ...
        assert!(variable_token_factory::supply(var_token_address) == amount_to_mint,
            TEST_SUCCESS);
        assert!(variable_token_factory::maximum(var_token_address)
        ...
        assert!(variable_token_factory::balance_of(signer::address_of(token_receiver),
                var_token_address) == amount_to_mint,
            TEST_SUCCESS);
        assert!(variable_token_factory::scale_balance_of(signer::address_of(token_receiver),
                var_token_address)
        ...
        variable_token_factory::burn(signer::address_of(token_receiver), amount_to_burn,
            reserve_index, var_token_address);

        // assert var token supply
        assert!(variable_token_factory::supply(var_token_address)
