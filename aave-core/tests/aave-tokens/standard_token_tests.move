...
module aave_pool::standard_token_tests {
    ...
    fun test_basic_flow(creator: &signer,) {
        ...
        mint_to_primary_stores(creator,
            metadata,
            vector[creator_address, aaron_address],
            vector[100, 50]);
        assert!(primary_fungible_store::balance(creator_address, metadata) == 100,
            TEST_SUCCESS);
        assert!(primary_fungible_store::balance(aaron_address, metadata) == 50, TEST_SUCCESS);

        set_primary_stores_frozen_status(creator, metadata, vector[creator_address, aaron_address], true);
        assert!(primary_fungible_store::is_frozen(creator_address, metadata), TEST_SUCCESS);
        assert!(primary_fungible_store::is_frozen(aaron_address, metadata), TEST_SUCCESS);

        transfer_between_primary_stores(creator,
            metadata,
            vector[creator_address, aaron_address],
            vector[aaron_address, creator_address],
            vector[10, 5]);
        assert!(primary_fungible_store::balance(creator_address, metadata) == 95,
            TEST_SUCCESS);
        assert!(primary_fungible_store::balance(aaron_address, metadata) == 55, TEST_SUCCESS);

        set_primary_stores_frozen_status(creator, metadata, vector[creator_address, aaron_address],
            false);
        assert!(!primary_fungible_store::is_frozen(creator_address, metadata), TEST_SUCCESS);
        assert!(!primary_fungible_store::is_frozen(aaron_address, metadata), TEST_SUCCESS);

        let fa =
            withdraw_from_primary_stores(creator,
                metadata,
                vector[creator_address, aaron_address],
                vector[25, 15]);
        assert!(fungible_asset::amount(&fa) == 40, TEST_SUCCESS);
        deposit_to_primary_stores(creator, &mut fa, vector[creator_address, aaron_address],
            vector[30, 10]);
        fungible_asset::destroy_zero(fa);

        burn_from_primary_stores(creator,
            metadata,
            vector[creator_address, aaron_address],
            vector[100, 50]);
        assert!(primary_fungible_store::balance(creator_address, metadata) == 0,
            TEST_SUCCESS);
        assert!(primary_fungible_store::balance(aaron_address, metadata) == 0, TEST_SUCCESS);
    }
