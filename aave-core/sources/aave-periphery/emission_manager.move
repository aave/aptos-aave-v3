module aave_pool::emission_manager {
    ...
    public fun configure_assets(
        ...
        for (i in 0..vector::length(&config)) {
            ...
            assert!(*smart_table::borrow(&emission_manager_data.emission_admins, reward)
                == signer::address_of(account),
                ONLY_EMISSION_ADMIN);
        };
        rewards_controller::configure_assets(account, config, emission_manager_data.rewards_controller);
    }

    public fun set_pull_rewards_transfer_strategy(
        caller: &signer, reward: address, pull_rewards_transfer_strategy: PullRewardsTransferStrategy,
    ) acquires EmissionManagerData {
        check_is_emission_admin(reward);

        let emission_manager_data =
            borrow_global_mut<EmissionManagerData>(emission_manager_address());
        rewards_controller::set_pull_rewards_transfer_strategy(caller, reward,
            pull_rewards_transfer_strategy, emission_manager_data.rewards_controller);
    }

    public fun set_staked_token_transfer_strategy(
        caller: &signer, reward: address, staked_token_transfer_strategy: StakedTokenTransferStrategy,
    ) acquires EmissionManagerData {
        check_is_emission_admin(reward);

        let emission_manager_data =
            borrow_global_mut<EmissionManagerData>(emission_manager_address());
        rewards_controller::set_staked_token_transfer_strategy(caller, reward,
            staked_token_transfer_strategy, emission_manager_data.rewards_controller);
    }
    ...
    public fun set_reward_oracle(
        ...
        check_is_emission_admin(reward);
        let emission_manager_data = borrow_global<EmissionManagerData>(
            emission_manager_address());
        rewards_controller::set_reward_oracle(caller, reward, reward_oracle,
    ...
    public fun set_distribution_end(
        ...
        let emission_manager_data = borrow_global<EmissionManagerData>(
            emission_manager_address());
        rewards_controller::set_distribution_end(caller,
            asset,
            reward,
    ...
    public fun set_emission_per_second(
        ...
        for (i in 0..vector::length(&rewards)) {
            assert!(*smart_table::borrow(&emission_manager_data.emission_admins, *vector::borrow(
                        &rewards, i))
                == signer::address_of(caller),
                ONLY_EMISSION_ADMIN);
        };
        rewards_controller::set_emission_per_second(caller,
            asset,
            rewards,
    ...
    public fun set_claimer(account: &signer, user: address, claimer: address) acquires EmissionManagerData {
        check_admin(signer::address_of(account));
        let emission_manager_data = borrow_global<EmissionManagerData>(
            emission_manager_address());
        rewards_controller::set_claimer(user, claimer, emission_manager_data.rewards_controller);
    }
    ...
    public fun set_rewards_controller(account: &signer, controller: address) acquires EmissionManagerData {
        check_admin(signer::address_of(account));
        let emission_manager_data =
            borrow_global_mut<EmissionManagerData>(emission_manager_address());
