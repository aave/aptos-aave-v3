module aave_pool::ecosystem_reserve_v2 {
    ...
    fun only_admin_or_recipient(account: address, stream_id: u256) acquires EcosystemReserveV2Data {
        assert!(acl_manage::is_funds_admin(account) || is_recipient(account, stream_id),
            NOT_FUNDS_ADMIN);
    }
    ...
    fun stream_exists(stream_id: u256) acquires EcosystemReserveV2Data {
        ...
        assert!(smart_table::contains(&ecosystem_reserve_v2_data.streams, stream_id),
            ESTREAM_NOT_EXISTS);
        let stream_item =
            smart_table::borrow(&ecosystem_reserve_v2_data.streams, stream_id);
    ...
    public fun ecosystem_reserve_v2_data_object(): Object<EcosystemReserveV2Data> {
        object::address_to_object<EcosystemReserveV2Data>(ecosystem_reserve_v2_data_address())
    }
    ...
    public fun balance_of(stream_id: u256, who: address): u256 acquires EcosystemReserveV2Data {
        ...
        let (sender, recipient, deposit, _, _, _, remaining_balance, rate_per_second) = stream::get_stream(
            stream_item);
    ...
    fun create_stream(
        ...
        assert!(recipient != ecosystem_reserve_v2_data_address(),
        ...
        let ecosystem_reserve_v2_data =
            borrow_global_mut<EcosystemReserveV2Data>(ecosystem_reserve_v2_data_address());
        let stream_item =
            stream::create_stream(deposit,
                rate_per_second,
                deposit,
                start_time,
                stop_time,
        ...
        smart_table::upsert(&mut ecosystem_reserve_v2_data.streams, stream_id, stream_item);

        ecosystem_reserve_v2_data.next_stream_id = ecosystem_reserve_v2_data.next_stream_id
            + 1;

        event::emit(CreateStream {
                stream_id,
    ...
    fun withdraw_from_stream(sender: &signer, stream_id: u256, amount: u256): bool acquires EcosystemReserveV2Data {
        stream_exists(stream_id);
        check_is_funds_admin(signer::address_of(sender));
        ...
        let (_, recipient, _, _, _, _, remaining_balance, _) = stream::get_stream(
            stream_item);
    ...
    fun cancel_stream(sender: &signer, stream_id: u256): bool acquires EcosystemReserveV2Data {
        ...
        let (stream_sender, recipient, _, token_address, _, _, _, _) = stream::get_stream(
            stream_item);
        ...
        if (recipient_balance > 0) {
            underlying_token_factory::transfer_from(stream_sender, recipient,
                (recipient_balance as u64), token_address);
        };

        event::emit(CancelStream {
                stream_id,
    ...
    fun test_basic_flow(
        ...
        let stream_id =
            create_stream(periphery_account,
                recipient,
                deposit,
                token_address,
                start_time,
    ...
    fun test_get_stream(
        ...
        let (stream_sender,
            ...
            stream_rate_per_second) = get_stream(stream_id);
